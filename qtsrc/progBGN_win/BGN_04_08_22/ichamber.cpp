...
void Ichamber::init()
{
    ...
    preSingleTimer = new QTimer(this);
    connect(preSingleTimer,&QTimer::timeout,this,&Ichamber::preSingleTimeOut);
    preSingleTimer->setSingleShot(true);

    for(volatile long i=0; i<500000000;i++){}
    emit writeToComSig(h03);
    for(volatile long i=0; i<500000000;i++){}
    emit writeToComSig(h03);
    preSingleTimer->start(500);
}
...
void Ichamber::preSingleTimeOut() {
    //*******************************************************************
    emit writeToComSig(h03); //k1+k9
