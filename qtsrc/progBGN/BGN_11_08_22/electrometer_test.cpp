...
//*************************************** Измерение тока ********************************************
    ...
    if (time==heatingTime+16*timeoutTest+5*averCounts)
    {
        ui->progressBar->setValue(60);
    }
    if (time==heatingTime+17*timeoutTest+5*averCounts)
    {
        ui->progressBar->setValue(70);
    }
    if (time==heatingTime+18*timeoutTest+5*averCounts)
    {
        ui->progressBar->setValue(80);
    }
    if (time==heatingTime+19*timeoutTest+5*averCounts)
    {
        ui->progressBar->setValue(90);
    }
    if (time>=heatingTime+19*timeoutTest+5*averCounts && time<=heatingTime+19*timeoutTest+6*averCounts)
    {
        //ui->progressBar->setValue(40+10*(time-(heatingTime+20*timeoutTest+5*averCounts)));
        y+=averageADC;
        if(time==heatingTime+19*timeoutTest+6*averCounts)
        {
            l1=y/(averCounts+1); // зреднее значение АЦП в разрядах
            U_Ioff=(l1-ADC0);
