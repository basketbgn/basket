...
void Gamma_BDG_verify::init()
{
    ...
    ///////////////////////////     set presetted sensibility from TOROPA ///////////////////
    timSens = new QTimer(this);
    connect(timSens, &QTimer::timeout, [&]{
        timSens->stop();
        QString str = QString::number(bdg->getCurrentToropaSensivity(),'g',3) + " (имп/с)/(Зв/с)";
        ui->label_10->setText(str);

        QString sensibility = ui->label_10->text(); // Чувствительность в Зависимости от ComboBox-a
...
//            else if (MesParam==5){qStr="UPDATE gammaBlock SET sensibilityMED='%1' WHERE nuclide='%2'";}
            ...
            qStr="UPDATE gammaBlock SET sensibilityMAED='%1' WHERE nuclide='%2'";
            QString qStr1=qStr.arg(ui->label_10->text()).arg(chamberName);
            qDebug()<<chamberName;
...
void Gamma_BDG_verify::timeOut()
{
    ...
    //------------------------------------Мощность дозы-------------------------------------------------------
        if(ui->checkBox->isChecked()) {
            I = double(bdg->IblockCR_Res); //считываем измерение скорости счета ФОНА
        } else {
            I = double(bdg->IblockCR_ResGamma); //считываем измерение скорости счета с учетом ФОНА
        }
...
void Gamma_BDG_verify::on_pushButton_5_clicked() // запись фоновой скорости счета
{
    QString ss = ui->label->text(); // скорость счета
    if(QSqlDatabase::contains("myDB"))
    {
        QSqlDatabase db = QSqlDatabase::database("myDB");
        db.setDatabaseName("config.db");
        ...
        QSqlQuery query(db);
        QString qStr="";
        qStr="UPDATE backgroundgamma SET backgroundGamma='%1'";

        QString qStr1=qStr.arg(ui->label->text());
        qDebug()<<chamberName;
        if(!query.exec(qStr1)){qDebug()<<"unable execute query UPDATE backgroundGamma";db.lastError().text();}
        db.close();
    }
    QMessageBox::warning(this,"Внимание","Значение фоновой скорости счета было изменено",QMessageBox::Ok);
}

void Gamma_BDG_verify::on_pushButton_6_clicked() // считывание чувствительности из блока
{
    //RS485 modbus;
    if(bdg->getBlockName().data()[0] == "К"){ //Кобальт
        bdg->readBdgSensivity(0);
    } else if(bdg->getBlockName().data()[0] == "Ц") { // Цезий
        bdg->readBdgSensivity(1);
    } else if(bdg->getBlockName().data()[0] == "А") { // Америций
        bdg->readBdgSensivity(2);
    }
    //system("sleep 1");
    timSens->start(500);
    //! TODO: перевести в (имп/с)/(мкЗв/ч) и записать ui->lineEdit_2
}
