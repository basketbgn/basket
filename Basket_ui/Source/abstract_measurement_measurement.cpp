...
{
    //проверяем размерность по времени
    switch (DimensionForTime) {
...
void Abstract_measurement_measurement::doseShow(double z, QString &fig4) const
{
    if (QString::number(z,'f',3).size()<6) {
        fig4 = (QString::number(z,'f',3));  // отображение 1,111
    }
    else if (QString::number(z,'f',3).size()>=6 && QString::number(z,'f',3).size()<7) {
        fig4 = (QString::number(z,'f',2));  // отображение 11,11
    }
    else if (QString::number(z,'f',3).size()>=7 && QString::number(z,'f',3).size()<8) {
        fig4 = (QString::number(z,'f',1));  // отображение 111,1
    }
    else if (QString::number(z,'f',3).size()>=8) {
        fig4 = (QString::number(z,'f',0));  // отображение 1111
    }
}
...
void Abstract_measurement_measurement::doseTo4(double z1,
                                               ...
                                               bool PP,
                                               bool Fluence) {
    //измеряем не Плотность Потока
    if(!PP) {
        double z = z1*1e12; //  умножаем на 10 в 12
        if (z < 1e3) {
            // отображение (пГр/с) или (пГр)
            dim = "п" + dimension;
            doseShow(z, fig4); // функция отображения значения величины
        } else if (z >= 1e3 && z < 1e6) {
            dim = "н" + dimension;  // отображение
            z = z/1e3;
            doseShow(z, fig4); // функция отображения значения величины
        } else if (z >= 1e6 && z < 1e9) {
            dim = "мк" + dimension;  // отображение
            z = z/1e6;
            doseShow(z, fig4); // функция отображения значения величины
        } else if (z >= 1e9 && z < 1e12) {
            dim = "м" + dimension;  // отображение
            z = z/1e9;
            doseShow(z, fig4); // функция отображения значения величины
        } else if (z >= 1e12) {
            dim = dimension;  // отображение
            z = z/1e12;
            doseShow(z, fig4);
        }
    } else {
        //измеряем плотность потока
        if(!Fluence) {
            dim = dimension;
            fig4 = (QString::number(z1,'f',2));
        } else {
            //измеряем флюенс (интеграл по времени от плотности потока)
            dim = dimension + "/кв.см";
            fig4 = QString::number(z1,'f',0);
        }
    }
}

void Abstract_measurement_measurement::autoMode(double dose, QTimer *timer, QString& time, QString& times) {
    ...
    time = (QString::number(automode_one_time_current));
    //если доходит до порога по времени
    if(automode_one_time_current == TimeAutomatic) {
        automode_one_time_current=0; // сбрасываем счетчик времени
        automode_times_current+=1;   // увеличиваем счетчик текущего измерения
        times = (QString::number(automode_times_current));
        ...
        //закончено последнее измерение
        if(automode_times_current==TimesAutomatic) {
